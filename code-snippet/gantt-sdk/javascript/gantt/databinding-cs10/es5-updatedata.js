...
for (let i = 0; i < 5; i++) {
    ...
    for (let j = 0; j < tempData1.length; j++) {
        let subtasks = {};
        /* tslint:disable:no-string-literal */
        subtasks['TaskID'] = tempData1[j].TaskID + x;
        subtasks['TaskName'] = tempData1[j].TaskName;
        subtasks['StartDate'] = tempData1[j].StartDate;
        subtasks['Duration'] = '10';
        subtasks['Progress'] = tempData1[j].Progress;
        subtasks['parentID'] = tempData1[j].parentID + x;
        updateData.push(subtasks);
    }
}