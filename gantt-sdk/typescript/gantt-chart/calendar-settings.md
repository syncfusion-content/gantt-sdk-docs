...
The Gantt Chart control supports advanced calendar configuration through the [calendarSettings](https://ej2.syncfusion.com/documentation/api/gantt#calendarsettings) property, enabling management of working hours, holidays, and task-specific scheduling. Calendar settings control how the Gantt Chart calculates task duration, determines working days, and schedules dependencies.

The [calendarSettings](https://ej2.syncfusion.com/documentation/api/gantt#calendarsettings) property contains two key configurations:
...
The [calendarSettings.projectCalendar](https://ej2.syncfusion.com/documentation/api/gantt/calendarSettings#projectcalendar) defines the default working hours and non-working days for the entire project. All tasks follow the project calendar unless assigned a task-specific calendar.
...
To assign a custom calendar to a task, first define the calendar in [calendarSettings.taskCalendar](https://ej2.syncfusion.com/documentation/api/gantt/calendarSettings#taskcalendar), then reference it using the [calendarId](https://ej2.syncfusion.com/documentation/api/gantt/taskFields#calendarid) property in the task data. Calendar exceptions allow defining specific dates with custom working hours, enabling team-specific scheduling adjustments such as split shifts or holidays that differ from the main schedule.
...
- **Weekends**: By default, weekends are treated as non-working days when [includeWeekend](https://ej2.syncfusion.com/documentation/api/gantt#includeweekend) is set to **false**.
