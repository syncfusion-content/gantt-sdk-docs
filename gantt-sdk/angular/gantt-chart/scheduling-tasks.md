...
description: Learn how to schedule scheduling tasks in Syncfusion Angular Gantt Chart with custom templates and event handling
platform: gantt-sdk
...
# Task Scheduling with Duration Units and Working Time in Angular Gantt Chart
...
export class AppComponent {
  ...
  public dataBound(args: object): void {
    const tasks = Array.isArray(this.ganttChart.dataSource)
      ? this.ganttChart.dataSource
      : [];
    tasks.forEach((task: any) => {
      if (
        !task.isManual &&
        task.StartDate &&
        new Date(task.StartDate) < this.projectStartDate
      ) {
