...
sf new syncfusion-angular-app --framework angular --template gantt
{% endhighlight %}
{% endtabs %}
...
When you run the `sf` command, the CLI prompts you to select the required project configuration. To create a Angular application with the Syncfusion<sup style="font-size:70%">&reg;</sup> `Gantt` component, select the following options:
...
The above selections generate a Angular application configured with the Syncfusion<sup style="font-size:70%">&reg;</sup> `Gantt` component. You can choose different values for language, theme, style format, MCP setup, and skills installation based on your project requirements.
