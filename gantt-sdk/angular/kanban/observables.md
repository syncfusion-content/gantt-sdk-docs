...
An [`Observable`](https://rxjs.dev/guide/observable) is used extensively by Angular since it provide significant benefits over techniques for event handling, asynchronous programming, and handling multiple values.

## Observable binding using async pipe

Kanban data can be consumed from an [`Observable`](https://rxjs.dev/guide/observable) object by piping it through an [`async`](https://angular.dev/api/common/AsyncPipe) pipe. The [`async`](https://angular.dev/api/common/AsyncPipe) pipe is used to subscribe the observable object and resolve with the latest value emitted by it.

## Data binding

The kanban expects an object from the [`Observable`](https://rxjs.dev/guide/observable). The emitted value should be an object with properties **result** and **count**.
...
> You should maintain the same [`Observable`](https://rxjs.dev/guide/observable) instance for every kanban action.
