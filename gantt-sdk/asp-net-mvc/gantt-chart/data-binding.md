...
We can pass additional parameters using [`addParams`](https://ej2.syncfusion.com/documentation/api/data/query#addparams) method of [`Query`](https://ej2.syncfusion.com/documentation/api/data/query) class. In server side we have inherited and shown the additional parameter value in Syncfusion<sup style="font-size:70%">&reg;</sup> DataManager class itself. We pass an additional parameter in load time using [`load`](https://ej2.syncfusion.com/documentation/api/gantt#load) event. We can also pass additional parameter to the CRUD model. Check the below code snippet to send additional parameter to Gantt.
...
During server interaction from the Gantt, some server-side exceptions may occur, and you can acquire those error messages or exception details in client-side using the [`actionFailure`](https://ej2.syncfusion.com/documentation/api/gantt#actionfailure) event.
...
You can use Gantt [`dataSource`](https://ej2.syncfusion.com/documentation/api/gantt#datasource) property to bind the data source to Gantt from external Fetch request. In the below code we have fetched the data source from the server with the help of Fetch request and provided that to `dataSource` property by using [`onSuccess`](https://ej2.syncfusion.com/documentation/api/base/ajax#onsuccess) event of the Fetch.
