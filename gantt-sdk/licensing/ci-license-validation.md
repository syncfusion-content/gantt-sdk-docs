...
description: Learn how to validate Syncfusion® license keys in CI or CD pipelines using the LicenseKeyValidator utility, secure variables, and automated tests.
keywords: validate license key in CI, continuous integration license validation, LicenseKeyValidator, GitHub Actions license validation, Azure Pipelines license validation, Jenkins license key
canonical: https://help.syncfusion.com/gantt-sdk/licensing/ci-license-validation
platform: gantt-sdk
...
* Open the `LicenseKeyValidation.ps1` script in a text or code editor. Replace the platform token with the target platform name (for example, `Blazor`, `MAUI`, `WPF`, `WinUI`, `UWP`). The default contents are shown below.
...
* Update the parameters in the script:
  ...
  **Platform:** Set /platform:"**UIComponent**" for v34.1.29 and later, or /platform:"**PlatformName**" for earlier versions (for example, `Blazor`, `MAUI`, `WPF`, `WinUI`, or `UWP`).

  **Version:** Change the value for /version: to the required version (for example, "26.2.4").

  **License Key:** Replace the value for /licensekey: with your actual license key (for example, "Your License Key").
...
* Register the license key properly by calling `RegisterLicense("Your License Key")` with the license key.

* Once the license key is registered, validate it by calling `ValidateLicense(Platform.<PlatformName>)`. This ensures that the license key is valid for the platform and version you are using. Replace `<PlatformName>` with the target platform (for example, `Blazor`, `MAUI`, `WPF`, `WinUI`, or `UWP`).
...
N> Use `Platform.UIComponent` for UI component license validation in v34.1.29 and later. The legacy platform-specific values such as `Platform.Blazor`, `Platform.MAUI`, `Platform.WPF`, `Platform.WinUI`, and `Platform.UWP` are not supported from v34.1.29 onwards.
...
* After registering the license key, validate it by calling `ValidateLicense(Platform.<PlatformName>, out var validationMessage)`. This ensures that the license key is valid for the platform and version you are using.

* The following example demonstrates how to register and validate the license key in an NUnit unit test project. Replace `<PlatformName>` with the target platform (for example, `Blazor` or `MAUI`).

{% tabs %}
{% highlight c# %}
public void TestSyncfusionLicense()
{
